...
def train_sfs(
    ...
    if 'FHN' in data_filepath:
        if 'FHNv' in data_filepath:
            ...
            ode_dt = 1e-3
    elif 'Kuramoto' in data_filepath:
        ode_dt = 0.05
    ...
    for epoch in range(1, learn_max_epoch+1):
        ...
        if epoch % 5 == 0 or epoch in [1, 2, 3, 4, 5, learn_max_epoch]:
...
def test_sfs(
        ...
        for input, _, internl_units in iter:
            ...
            if submodel == 'neural_ode':
                ...
                for i in range(n):
                    ...
                    if fast:
                        if mask_slow:
                            unit_fast_obs_list.append(model.node_f_evolve(unit_fast_obs_list[-1], torch.zeros_like(unit_slow_var_list[-2]), t, fast_dt=ode_dt, slow_dt=ode_dt*rho))
                        else:
                            unit_fast_obs_list.append(model.node_f_evolve(unit_fast_obs_list[-1], unit_slow_var_list[-2], t, fast_dt=ode_dt, slow_dt=ode_dt*rho))
                    else:
                        unit_fast_obs_list.append(torch.zeros_like(unit_fast_obs_list[-1]))
                    ...
                    true_total_obs_list.append(internl_units[i][...,:obs_dim].to(device)) # not scale
    ...
    for i in range(1, n+1):
        ...
        for j in range(50):
            ...
            tmp.append(np.mean((true_group-pred_group)**2)/np.var(true_group).item())
