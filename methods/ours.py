...
def train_ami(
    ...
    for epoch in iter:
        ...
        for x_t0, x_t1 in train_loader:
            ...
            if bi_info:
                latent_reverse, embed2 = model.forward(x_t1, direct='reverse')
                latent_t0 = model.enc(x_t0, direct='reverse')
        ...
        with torch.no_grad():
            ...
            if bi_info:
                ...
                if is_print: 
                    iter.set_description(f'Tau[{tau}] | epoch[{epoch}/{max_epoch}] val-MSE: prior={prior_loss:.5f}, reverse={reverse_loss:.5f}, symmetry={symmetry_loss:.5f}')
            else:
                if is_print:
